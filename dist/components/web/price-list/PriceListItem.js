...
var PriceListItem = function (props) {
    var variant = props.variant, image = props.image, price = props.price, title = props.title, subtitle = props.subtitle, action = props.action, path = props.path, url = props.url, src = props.src;
    ...
    return (react_1.default.createElement("li", { className: 'w-full' },
        react_1.default.createElement("button", { onClick: handleClick, className: (0, frontend_shadcn_1.cn)('w-full flex justify-between items-center rounded-xl p-4 focus:outline-none hover:bg-muted', variant == 'fill' && 'bg-muted p-6 rounded-lg', variant == 'outline' && 'border border-border p-6 rounded-lg') },
