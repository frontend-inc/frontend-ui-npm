...
var ApiQuery = /** @class */ (function () {
  ...
  ApiQuery.prototype.where = function (searchParams) {
    var _a = searchParams || {},
      _b = _a.sort_by,
      sort_by = _b === void 0 ? "id" : _b,
      _c = _a.sort_direction,
      sort_direction = _c === void 0 ? "desc" : _c,
      keywords = _a.keywords,
      filters = _a.filters,
      _d = _a.page,
      page = _d === void 0 ? 1 : _d,
      _e = _a.per_page,
      per_page = _e === void 0 ? 20 : _e,
      rest = __rest(_a, [
        "sort_by",
        "sort_direction",
        "keywords",
  ...
  ApiQuery.prototype.AND_filter = function (filter) {
    this._filters = __assign(__assign({}, this._filters), {
      AND: __spreadArray(
        __spreadArray([], this._filters["AND"] || [], true),
        [filter],
        false
      ),
    });
  ...
  ApiQuery.prototype.OR_filter = function (filter) {
    this._filters = __assign(__assign({}, this._filters), {
      OR: __spreadArray(
        __spreadArray([], this._filters["OR"] || [], true),
        [filter],
        false
      ),
    });
  ...
  ApiQuery.prototype.url = function () {
    ...
    if (this._sort_by && this._sort_direction) {
      searchParams = __assign(__assign({}, searchParams), {
        order: "".concat(this._sort_by, ":").concat(this._sort_direction),
      });
    }
    ...
    if (
      ...
      Object.keys(this._filters).forEach(function (where) {
        ...
        andOrfilters === null || andOrfilters === void 0
          ? void 0
          : andOrfilters.forEach(function (filter) {
              if (_this.isValidFilter(filter)) {
                ...
                if (where == "AND") {
                  andFilters.push(
                    "".concat(field, ":").concat(operator, ":").concat(value)
                  );
                }
                if (where == "OR") {
                  orFilters.push(
  ...
  ApiQuery.prototype.parseURL = function (routerParams) {
    if (routerParams === void 0) {
      routerParams = {};
    }
    var keywords = routerParams.keywords,
      page = routerParams.page,
      per_page = routerParams.per_page,
      filterParams = routerParams.filters,
      order = routerParams.order;
    var _a = order ? order.split(":") : [],
      sort_by = _a[0],
      sort_direction = _a[1];
    var filters = {};
    ...
    if (filterParams) {
      ...
      if (andPart) {
        ...
        andFilterArray = andFilters.split(filterRegex).map(function (filter) {
          ...
          return (
            (_a = {}), (_a[field] = ((_b = {}), (_b[operator] = value), _b)), _a
          );
        });
