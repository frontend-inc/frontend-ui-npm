...
var reorder = function (items, startIndex, endIndex) {
  ...
  var sorted = result.map(function (item, index) {
...
var reorderList = function (items, source, destination) {
  var current = items.find(function (i) {
    return i.id === source.droppableId;
  });
  var next = items.find(function (i) {
    return i.id === destination.droppableId;
  });
  var target = current.items[source.index];
  // moving to same list
  if (source.droppableId === destination.droppableId) {
    var reordered_1 = (0, exports.reorder)(
      __spreadArray([], current.items, true),
      source.index,
      destination.index
    );
